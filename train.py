...
from models.svdd import SVDD, SVDD_Plus, BCEFocalLoss
...
n_samples = 0
C = torch.zeros(32).to(DEVICE)
embeds = []
with torch.no_grad():
    for data, label in train_loader:
        data = data.to(DEVICE)
        label = label.to(DEVICE)
        _, embedding, _ = base_model(data)
...
dist = np.sum((embeds - C.cpu().numpy()) ** 2, axis=1)
euclidean_norm = np.sqrt(dist)
R = np.quantile(euclidean_norm, 1 - 0.05)
...
# SVDD model
model = TCN_VAE_AD(base_model, SVDD(C, R)).double().to(DEVICE)

optimizer = optim.SGD(model.parameters(), lr=LEARNING_RATE)
lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=EPOCHS)
mse_criterion = nn.MSELoss().to(DEVICE)
bce_criterion = BCEFocalLoss().to(DEVICE)
...
for epoch in range(EPOCHS):
    ...
    for batch_idx, (data, label) in enumerate(data_bar):
        ...
        BCE = bce_criterion(anomaly_score, label.long())
